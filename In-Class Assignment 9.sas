...
libname sql"/home/u60682576/sql";
...
proc sql;
	SELECT CONT, SUM(pop) AS total_population
	FROM sql.demographics
	GROUP BY CONT;
quit;
...
proc sql;
	SELECT MAX(pop) AS maximum_population_value, *
	FROM sql.demographics;
quit;
...
proc sql;
	SELECT name AS country, MAX(pop) AS maximum_population_value
	FROM sql.demographics
	WHERE pop = 
	(SELECT MAX(pop) AS maximum_population_value
	FROM sql.demographics);
